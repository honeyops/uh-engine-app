...
>(
  (
    ...
    // Start with defaultOpen to match server render, then update from cookie after mount
    // We use openProp and setOpenProp for control from outside the component.  
    const [_open, _setOpen] = React.useState(defaultOpen)
    const open = openProp ?? _open

    // Restore state from cookie after mount to avoid hydration mismatch
    React.useEffect(() => {
      if (!openProp && typeof document !== "undefined") {
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "md:peer-data-[variant=inset]:mt-2 md:peer-data-[variant=inset]:mr-2 md:peer-data-[variant=inset]:mb-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow",
        className
      )}
      {...props}
    />
