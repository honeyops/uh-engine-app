...
export async function GET(request: Request) {
	...
	try {
		const res = await fetch(`${getApiBase()}/sources/metadata/schemas?` + new URLSearchParams({ db: database }), { cache: 'no-store' })
		if (!res.ok) return NextResponse.json({ database, schemas: [] })
		const data = await res.json()
		return NextResponse.json(data)
	} catch {
		return NextResponse.json({ database, schemas: [] })
	}
}
