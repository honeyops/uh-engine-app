...
export async function GET(request: Request) {
	...
	try {
		const params = new URLSearchParams({ db: database, schema })
		const res = await fetch(`${getApiBase()}/sources/metadata/tables?` + params, { cache: 'no-store' })
		if (!res.ok) return NextResponse.json({ database, schema, tables: [] })
		const data = await res.json()
		return NextResponse.json(data)
	} catch {
		return NextResponse.json({ database, schema, tables: [] })
	}
}
